...
Download the immutable 0.8.5 package:

```bash
curl -fL \
  -o vendor/edgepg-0.8.5.tgz \
  https://github.com/LogicMateCA/edgepg/releases/download/v0.8.5/edgepg-0.8.5.tgz
```
...
printf '%s  %s\n' \
  ...
  'vendor/edgepg-0.8.5.tgz' | sha256sum -c -
```

Expected output:

```text
vendor/edgepg-0.8.5.tgz: OK
```

On macOS, use `shasum -a 256 vendor/edgepg-0.8.5.tgz`. On Windows PowerShell, use `Get-FileHash -Algorithm SHA256 vendor/edgepg-0.8.5.tgz` and compare the value exactly.

Install the verified local file:

```bash
npm install ./vendor/edgepg-0.8.5.tgz
```

Keep the package and lockfile in your controlled build inputs. Do not replace 0.8.5 with an unverified mutable URL during CI.
...
- the package version is `0.8.5` in `node_modules/edgepg/package.json`;
