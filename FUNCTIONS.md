...
|---|---:|---|---|
| `bool_or(boolean)` | ✅ | mixed true/false/NULL, all-NULL, empty input, `GROUP BY`, nullable `LEFT JOIN` expressions, prepared parameters, `FILTER`, `DISTINCT`, transaction, OID 16 | `0.8.1-rc.17` |
| `bool_and(boolean)` | ✅ | same matrix as `bool_or`; PostgreSQL NULL/empty semantics and OID 16 | `0.8.1-rc.17` |
| `every(boolean)` | ✅ | PostgreSQL synonym of `bool_and`, including NULL/empty and grouped forms | `0.8.1-rc.17` |
| `count(*)`, `count(expr)` | ✅ | grouped, correlated scalar, empty input, cast to integer, bigint result metadata | retained in `0.8.1-rc.17` |
| `sum(...)`, `avg(...)` | ✅ | common grouped/window forms, integer/numeric result OIDs and NULL input | retained in `0.8.1-rc.17` |
| `array_agg(...)` | ✅ | ordered, `DISTINCT`, nested arrays, `FILTER` inside `COALESCE`, empty input/decode, dimensional validation and array OIDs | `0.8.1-rc.17` |
| `json_agg(...)`, `jsonb_agg(...)` | ✅ | `ORDER BY`, `FILTER`, `DISTINCT`, correlated aggregates, empty fallback and OIDs 114/3802 | retained in `0.8.1-rc.17` |
