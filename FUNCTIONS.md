...
|---|---:|---|---|
| `bool_or(boolean)` | ✅ | mixed true/false/NULL, all-NULL, empty input, `GROUP BY`, nullable `LEFT JOIN` expressions, prepared parameters, `FILTER`, `DISTINCT`, transaction, OID 16 | retained in `0.8.4` |
| `bool_and(boolean)` | ✅ | same matrix as `bool_or`; PostgreSQL NULL/empty semantics and OID 16 | retained in `0.8.4` |
| `every(boolean)` | ✅ | PostgreSQL synonym of `bool_and`, including NULL/empty and grouped forms | retained in `0.8.4` |
