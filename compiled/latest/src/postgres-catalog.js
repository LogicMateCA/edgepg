import{quoteCatalogIdentifier as be}from"./sql-quoting";import{ensureRoleSchema as K}from"./roles";import{ensureCustomTypeCatalogSchema as He}from"./custom-types";import{ensureRowSecuritySchema as Q}from"./row-security";import{ensureTableConstraintSchema as re}from"./table-constraints";import{ensureMaterializedViewSchema as Re}from"./materialized-views";import{ensureEventTriggerSchema as je,pgEventTriggerDependUnionSql as fe,pgEventTriggerSharedDependUnionSql as he,pgEventTriggerViewSql as Ie}from"./event-triggers";import{compactViewDefinition as le}from"./view-definitions";import{ensureDefaultPrivilegeSchema as We}from"./default-privileges";import{ensureAggregateCatalogSchema as De}from"./aggregates";import{ensureOperatorTypeCatalogSchema as we,operatorTypeCatalogViewSql as Ue}from"./operator-types";import{ensureCollationSchema as ve}from"./collations";import{ensureTextSearchCatalogSchema as Me,textSearchCatalogViewSql as Fe}from"./text-search";import{conversionCatalogViewSql as $e,conversionDependencyUnionSql as xe,conversionSharedDependViewSql as Xe,ensureConversionCatalogSchema as qe}from"./conversions";import{ensureSequenceDefaultsSchema as pe,ensureSequenceSchema as ce}from"./sequences";import{d1CacheKey as f}from"./d1-cache-key";import{PG18_TYPE_IO_CATALOG_ROWS as Je,PG18_TYPE_IO_PROC_ROWS as Be}from"./postgres-builtins";const X=33,F=3e8,me=6e8,Z=6e4,Ge=12e4,ke=125e3,Pe=50,ge=`CREATE INDEX IF NOT EXISTS __edgepg_pg_tables_table_name
  ...
  PRIMARY KEY (phase, kind, schema_name, object_name)
)`,Ne=[["dependencies_json","dependencies_json TEXT NOT NULL DEFAULT '[]'"],["routine_dependencies_json","routine_dependencies_json TEXT NOT NULL DEFAULT '[]'"],["view_definition","view_definition TEXT"],["compact_view_definition","compact_view_definition TEXT"],["relation_oid","relation_oid INTEGER"],["owner_name","owner_name TEXT NOT NULL DEFAULT 'edgepg'"],["relation_natts","relation_natts INTEGER NOT NULL DEFAULT 0"],["relation_pg_types_json","relation_pg_types_json TEXT NOT NULL DEFAULT '[]'"],["relation_collations_json","relation_collations_json TEXT NOT NULL DEFAULT '[]'"],["reloptions_json","reloptions_json TEXT NOT NULL DEFAULT '[]'"],["index_nulls_not_distinct","index_nulls_not_distinct INTEGER NOT NULL DEFAULT 0"]],h=new WeakSet,b=new WeakMap,A=new WeakSet,d=new WeakSet,u=new WeakSet,H=new WeakMap,$=new WeakMap;function zn(e){const n=f(e);return d.has(n)&&u.has(n)}async function Ve(e,n={}){const t=f(e);if(d.has(t)){if(!n.validate&&G(H,t))return;if(await q(e,"__edgepg_pg_tables")){H.set(t,Date.now());return}B(e)}if(await Qe(e)){d.add(t),H.set(t,Date.now());return}if(await q(e,"__edgepg_pg_tables")){await Ye(e,{views:!1}),d.add(t),H.set(t,Date.now());return}n.roleSchemaReady||await K(e),await re(e),await e.batch([e.prepare(`CREATE TABLE IF NOT EXISTS __edgepg_pg_tables (
      ...
      ON __edgepg_pg_tables(relation_oid)`),e.prepare(ge),e.prepare(`CREATE TABLE IF NOT EXISTS __edgepg_table_inheritance (
      ...
      END`),...k(e,"postgres-catalog-table-storage")]),d.add(t),H.set(t,Date.now())}async function Kn(e,n={}){const t=f(e);if(u.has(t)){if(!n.validate&&G($,t))return;if(await J(e)){y(t);return}B(e)}if(A.has(t)||await ne(e)){await Ae(e),A.add(t),d.add(t),u.add(t),y(t);return}await Ve(e,n),await Ae(e),u.add(t),y(t)}async function Ye(e,n={}){const t=f(e),a=n.views!==!1;if(!(a&&h.has(t)&&!n.validate&&Date.now()-(b.get(t)||0)<Z)&&!(!a&&A.has(t)&&!n.validate&&G($,t))){if(!n.validate&&a&&await Se(e)){h.add(t),b.set(t,Date.now()),A.add(t),d.add(t),u.add(t),y(t);return}if(!n.validate&&!a&&await ne(e)){A.add(t),d.add(t),u.add(t),y(t);return}await Ke(e,()=>ze(e,n))}}async function ze(e,n={}){const t=f(e),a=n.views!==!1;if(a&&h.has(t)){const c=b.get(t)||0;if(!n.validate&&Date.now()-c<Z)return;if(await Q(e),await Te(e)){b.set(t,Date.now());return}B(e)}if(a&&(await Q(e),await ce(e),await pe(e),await e.batch([e.prepare(`CREATE TABLE IF NOT EXISTS __edgepg_table_inheritance (
        ...
        ON __edgepg_table_inheritance(parent_physical_name,ordinal)`)])),a&&await Se(e)){h.add(t),b.set(t,Date.now()),A.add(t),d.add(t),u.add(t),y(t);return}const o=A.has(t)?!a&&!n.validate&&G($,t)?!0:await J(e):await ne(e);if(A.has(t)&&!o&&B(e),!a&&o){A.add(t),d.add(t),u.add(t),y(t);return}if(o)A.add(t),d.add(t),u.add(t);else{await K(e),await We(e),await He(e),await Q(e),await re(e),await De(e),await we(e),await ve(e),await Me(e),await qe(e),await ce(e),await pe(e),await e.batch([e.prepare(`CREATE TABLE IF NOT EXISTS __edgepg_pg_tables (
      relation_oid INTEGER UNIQUE,
      schema_name TEXT NOT NULL,
      table_name TEXT NOT NULL,
      physical_name TEXT NOT NULL UNIQUE,
      ...
      ('plpgsql','plpgsql',1,'CREATE LANGUAGE plpgsql',CURRENT_TIMESTAMP)`)]);const c=await e.prepare("PRAGMA table_info('__edgepg_pg_tables')").all();c.results.some(_=>_.name==="relation_oid")||await e.prepare("ALTER TABLE __edgepg_pg_tables ADD COLUMN relation_oid INTEGER").run(),c.results.some(_=>_.name==="alterations_json")||await e.prepare("ALTER TABLE __edgepg_pg_tables ADD COLUMN alterations_json TEXT NOT NULL DEFAULT '[]'").run(),c.results.some(_=>_.name==="shape_changes_json")||await e.prepare("ALTER TABLE __edgepg_pg_tables ADD COLUMN shape_changes_json TEXT NOT NULL DEFAULT '[]'").run(),c.results.some(_=>_.name==="current_columns_json")||await e.prepare("ALTER TABLE __edgepg_pg_tables ADD COLUMN current_columns_json TEXT").run(),c.results.some(_=>_.name==="current_pg_types_json")||await e.prepare("ALTER TABLE __edgepg_pg_tables ADD COLUMN current_pg_types_json TEXT").run(),c.results.some(_=>_.name==="current_attnums_json")||await e.prepare("ALTER TABLE __edgepg_pg_tables ADD COLUMN current_attnums_json TEXT").run(),c.results.some(_=>_.name==="max_attnum")||await e.prepare("ALTER TABLE __edgepg_pg_tables ADD COLUMN max_attnum INTEGER NOT NULL DEFAULT 0").run(),c.results.some(_=>_.name==="current_not_null_json")||await e.prepare("ALTER TABLE __edgepg_pg_tables ADD COLUMN current_not_null_json TEXT").run(),c.results.some(_=>_.name==="current_defaults_json")||await e.prepare("ALTER TABLE __edgepg_pg_tables ADD COLUMN current_defaults_json TEXT").run(),c.results.some(_=>_.name==="generated_columns_json")||await e.prepare("ALTER TABLE __edgepg_pg_tables ADD COLUMN generated_columns_json TEXT NOT NULL DEFAULT '[]'").run(),c.results.some(_=>_.name==="persistence")||await e.prepare("ALTER TABLE __edgepg_pg_tables ADD COLUMN persistence TEXT NOT NULL DEFAULT 'permanent'").run(),c.results.some(_=>_.name==="reloptions_json")||await e.prepare("ALTER TABLE __edgepg_pg_tables ADD COLUMN reloptions_json TEXT NOT NULL DEFAULT '[]'").run(),c.results.some(_=>_.name==="replica_identity")||await e.prepare("ALTER TABLE __edgepg_pg_tables ADD COLUMN replica_identity TEXT NOT NULL DEFAULT 'd'").run(),c.results.some(_=>_.name==="foreign_server_name")||await e.prepare("ALTER TABLE __edgepg_pg_tables ADD COLUMN foreign_server_name TEXT").run(),c.results.some(_=>_.name==="foreign_options_json")||await e.prepare("ALTER TABLE __edgepg_pg_tables ADD COLUMN foreign_options_json TEXT NOT NULL DEFAULT '[]'").run(),c.results.some(_=>_.name==="foreign_column_options_json")||await e.prepare("ALTER TABLE __edgepg_pg_tables ADD COLUMN foreign_column_options_json TEXT NOT NULL DEFAULT '{}'").run();const R=await e.prepare("PRAGMA table_info('__edgepg_pg_objects')").all();await de(e,"__edgepg_pg_objects",R.results,Ne);const j=(await e.prepare(`SELECT phase,schema_name,object_name,view_definition
    ...
    SET compact_view_definition=?4 WHERE phase=?1 AND kind='view' AND schema_name=?2 AND object_name=?3`).bind(_.phase,_.schema_name,_.object_name,le(_.view_definition)).run();await jn(e),await e.prepare(`UPDATE __edgepg_pg_tables SET relation_oid=16383+ordinal
    WHERE relation_oid IS NULL`).run(),await e.prepare(`CREATE UNIQUE INDEX IF NOT EXISTS __edgepg_pg_tables_relation_oid
    ON __edgepg_pg_tables(relation_oid)`).run(),await e.prepare(ge).run(),await e.prepare(`CREATE TABLE IF NOT EXISTS __edgepg_oid_allocator (
  ...
  )`).run();const U=await e.prepare("PRAGMA table_info('__edgepg_schemas')").all();U.results.some(_=>_.name==="oid")||await e.prepare("ALTER TABLE __edgepg_schemas ADD COLUMN oid INTEGER").run(),U.results.some(_=>_.name==="acl_initialized")||await e.prepare("ALTER TABLE __edgepg_schemas ADD COLUMN acl_initialized INTEGER NOT NULL DEFAULT 0").run(),await e.prepare(`INSERT OR IGNORE INTO __edgepg_schemas(name, owner_name, oid) VALUES
    ...
    SET oid=COALESCE((SELECT MAX(oid)+1 FROM __edgepg_schemas), 20000) WHERE name=?1`).bind(_.name).run();await K(e),await Re(e),await je(e),await e.prepare(`UPDATE __edgepg_oid_allocator SET next_oid=MAX(next_oid,
    ...
    FROM __edgepg_sequences`).run(),await e.prepare(an()).run(),await e.prepare(`CREATE VIEW pg_catalog__pg_rewrite AS
    ...
    FROM __edgepg_materialized_views AS materialized WHERE materialized.relation_name<>''`).run(),await e.prepare(yn()).run(),await e.prepare(`CREATE VIEW pg_catalog__pg_enum AS
    ...
    WHERE types.kind='enum' AND identities.oid IS NOT NULL`).run();const p=D("domain.base_type");await e.prepare(`CREATE VIEW pg_catalog__pg_depend_base AS
    ...
    LEFT JOIN __edgepg_type_identities AS base_identity
      ON base_identity.name=replace(replace(${p},'.','__'),'[]','')
    ...
    SELECT classid,objid,objsubid,refclassid,refobjid,refobjsubid,deptype FROM (
      ...
      ${fe()}`),e.prepare($e()),e.prepare(Xe(he()))]),await e.prepare(Cn()).run(),await e.prepare(`CREATE VIEW pg_catalog__pg_index AS
    ...
    SELECT tables.relation_oid,object.relation_oid,0,
      ...
      (SELECT count(*) FROM pragma_index_info(CASE WHEN object.schema_name='public' THEN object.object_name
        ELSE object.schema_name || '__' || object.object_name END)),object.index_nulls_not_distinct
    FROM __edgepg_pg_objects AS object
    ...
    WHERE object.kind='index' AND object.relation_oid IS NOT NULL`).run(),await e.prepare(Rn()).run();const l=D("pg_type.value"),s=W(l),m=oe("pg_type.value"),g=ae("type_catalog"),E=D("COALESCE(json_extract(object.relation_pg_types_json, '$[' || view_column.cid || ']'), view_column.type)"),i=W(E),r=ae("view_type_catalog"),L=D("index_pg_type.value"),C=W(L),N=ae("index_type_catalog");await e.prepare(`CREATE VIEW pg_catalog__pg_attribute AS
    SELECT tables.relation_oid AS attrelid,
      ...
      CAST(column_name.value AS TEXT) AS attname, ${s} AS atttypid,
      ${m} AS atttypmod,${g},
    ...
    LEFT JOIN pg_catalog__pg_type AS type_catalog ON type_catalog.oid=${s}
    UNION ALL
    ...
    LEFT JOIN pg_catalog__pg_type AS index_type_catalog ON index_type_catalog.oid=${C}
    WHERE object.kind='index' AND object.relation_oid IS NOT NULL AND index_column.name IS NOT NULL`).run(),await e.prepare(_n()).run(),await e.prepare(`CREATE VIEW pg_catalog__pg_constraint AS
    ...
    JOIN __edgepg_schemas AS schemas ON schemas.name=tables.schema_name`).run();const T=Ue();await I(e,[e.prepare(T.pgAm),e.prepare(T.pgOperator),e.prepare(T.pgOpfamily),e.prepare(T.pgOpclass),e.prepare(T.pgAmop),e.prepare(T.pgAmproc),e.prepare(T.pgCast),e.prepare(T.pgCollation)]);const S=Fe();await I(e,[e.prepare(S.pgTsParser),e.prepare(S.pgTsTemplate),e.prepare(S.pgTsDict),e.prepare(S.pgTsConfig),e.prepare(S.pgTsConfigMap)]),await e.prepare(`CREATE VIEW pg_catalog__pg_description AS
    ...
    FROM __edgepg_pg_extensions`).run(),await e.prepare(gn()).run(),await e.prepare(Nn()).run(),await e.prepare(Sn()).run(),await I(e,[e.prepare(mn()),e.prepare(Tn()),e.prepare(dn()),e.prepare(An()),e.prepare(un()),e.prepare(Ln()),e.prepare(On())]),await e.prepare(`CREATE VIEW pg_catalog__pg_db_role_setting AS
    ...
    FROM __edgepg_pg_tables`).run(),await e.prepare(tn()).run(),await e.prepare(`CREATE VIEW information_schema__role_table_grants AS
    ...
    SELECT * FROM information_schema__role_column_grants`).run(),await e.prepare(nn()).run(),await e.prepare(`CREATE VIEW information_schema__sequences AS
    ...
    FROM __edgepg_foreign_constraints AS foreign_constraint
    JOIN __edgepg_pg_tables AS tables ON tables.physical_name=foreign_constraint.table_name
    JOIN json_each(foreign_constraint.columns_json) AS constraint_column`).run(),await en(e),h.add(t),b.set(t,Date.now()),A.add(t),d.add(t),u.add(t),y(t)}async function Ke(e,n){const t=crypto.randomUUID(),a=Date.now()+ke;for(await e.prepare(`CREATE TABLE IF NOT EXISTS __edgepg_catalog_install_leases (
    name TEXT PRIMARY KEY,owner TEXT NOT NULL,expires_at INTEGER NOT NULL)`).run();;){const o=Date.now();if(await e.prepare(`INSERT INTO __edgepg_catalog_install_leases(name,owner,expires_at)
      ...
      WHERE __edgepg_catalog_install_leases.expires_at<=?3`).bind(t,o+Ge,o).run(),(await e.prepare(`SELECT owner FROM __edgepg_catalog_install_leases
      WHERE name='postgres-catalog'`).first())?.owner===t)break;if(o>=a)throw new Error("timed out waiting for PostgreSQL catalog initialization");await new Promise(l=>setTimeout(l,Pe))}try{await n()}finally{await e.prepare(`DELETE FROM __edgepg_catalog_install_leases
      WHERE name='postgres-catalog' AND owner=?1`).bind(t).run()}}async function Se(e){try{const n=await e.prepare(`SELECT schema_version FROM __edgepg_catalog_meta
      WHERE key='postgres-catalog'`).first();return Number(n?.schema_version||0)===X&&await Te(e)}catch(n){if(/no such table:\s*__edgepg_catalog_meta/iu.test(n instanceof Error?n.message:String(n)))return!1;throw n}}async function ne(e){try{const n=await e.prepare(`SELECT key,schema_version FROM __edgepg_catalog_meta
      WHERE key IN ('postgres-catalog','postgres-catalog-storage')
      ORDER BY CASE key WHEN 'postgres-catalog' THEN 0 ELSE 1 END LIMIT 1`).first();return Number(n?.schema_version||0)===X&&await J(e)}catch(n){if(/no such table:\s*__edgepg_catalog_meta/iu.test(n instanceof Error?n.message:String(n)))return!1;throw n}}async function Qe(e){try{const n=await e.prepare(`SELECT key,schema_version FROM __edgepg_catalog_meta
      WHERE key IN ('postgres-catalog','postgres-catalog-storage','postgres-catalog-table-storage')
      ORDER BY CASE key WHEN 'postgres-catalog' THEN 0 WHEN 'postgres-catalog-storage' THEN 1 ELSE 2 END LIMIT 1`).first();return Number(n?.schema_version||0)===X&&await q(e,"__edgepg_pg_tables")}catch(n){if(/no such table:\s*__edgepg_catalog_meta/iu.test(n instanceof Error?n.message:String(n)))return!1;throw n}}async function q(e,n){return!!await e.prepare(`SELECT 1 AS present FROM sqlite_master
    ...
    WHERE type='table' AND name IN (?1,?2)`).bind("__edgepg_pg_tables","__edgepg_pg_objects").all(),t=new Set((n.results||[]).map(o=>o.name));return!t.has("__edgepg_pg_tables")||!t.has("__edgepg_pg_objects")?!1:(await e.prepare("PRAGMA table_info('__edgepg_pg_objects')").all()).results.some(o=>o.name==="index_nulls_not_distinct")}async function Te(e){if(!(await J(e)&&await te(e,"pg_catalog__pg_class")&&await te(e,"pg_catalog__pg_policies")&&await te(e,"information_schema__columns")))return!1;try{return await e.prepare(`SELECT indrelid,indexrelid,indisprimary,indisunique,indisvalid,indisready,
      indimmediate,indkey,indexprs,indpred,indisreplident,indnkeyatts,indnatts,indnullsnotdistinct
      FROM pg_catalog__pg_index LIMIT 0`).all(),await e.prepare(`SELECT schemaname,tablename,indexname,tablespace,indexdef
      FROM pg_catalog__pg_indexes LIMIT 0`).all(),await e.prepare(`SELECT oid,relname,relnamespace,reltype,relowner,relkind,relpersistence,
      relam,relispartition,relnatts,relhasindex,relrowsecurity,relforcerowsecurity,relispopulated,
      reloptions,relreplident,reltablespace,reltoastrelid,reloftype,relchecks,relhasrules,
      relhastriggers,relpages,reltuples,relallvisible,relallfrozen FROM pg_catalog__pg_class LIMIT 0`).all(),await e.prepare(`SELECT table_catalog,table_schema,table_name,column_name,ordinal_position,
      is_nullable,column_default,data_type,udt_schema,udt_name,character_maximum_length,
      ...
      is_generated,generation_expression FROM information_schema__columns LIMIT 0`).all(),!0}catch{return!1}}function B(e){const n=f(e);h.delete(n),b.delete(n),A.delete(n),d.delete(n),u.delete(n),H.delete(n),$.delete(n)}function G(e,n){return Date.now()-(e.get(n)||0)<Z}function y(e){const n=Date.now();H.set(e,n),$.set(e,n)}async function de(e,n,t,a){const o=new Set(t.map(p=>p.name));for(const[p,l]of a)if(!o.has(p))try{await e.prepare(`ALTER TABLE ${n} ADD COLUMN ${l}`).run(),o.add(p)}catch(s){if(!/duplicate column name/iu.test(s instanceof Error?s.message:String(s))||!(await e.prepare(`PRAGMA table_info('${n}')`).all()).results.some(g=>g.name===p))throw s;o.add(p)}}async function Ae(e){if(!await q(e,"__edgepg_pg_objects")){await e.batch([e.prepare(ee),e.prepare(`CREATE TABLE IF NOT EXISTS __edgepg_oid_allocator (
        id INTEGER PRIMARY KEY CHECK(id=1), next_oid INTEGER NOT NULL)`),e.prepare(`INSERT OR IGNORE INTO __edgepg_oid_allocator(id,next_oid)
        VALUES (1, MAX(
        ...
        AFTER INSERT ON __edgepg_pg_objects WHEN NEW.kind IN ('view','index') AND NEW.relation_oid IS NULL BEGIN
          UPDATE __edgepg_pg_objects SET relation_oid=(SELECT next_oid FROM __edgepg_oid_allocator WHERE id=1)
            WHERE phase=NEW.phase AND kind=NEW.kind AND schema_name=NEW.schema_name AND object_name=NEW.object_name;
          UPDATE __edgepg_oid_allocator SET next_oid=next_oid+1 WHERE id=1;
        END`)]);return}await e.prepare(ee).run();const n=await e.prepare("PRAGMA table_info('__edgepg_pg_objects')").all();n.results.some(o=>o.name==="index_nulls_not_distinct")||(await e.prepare("DROP VIEW IF EXISTS pg_catalog__pg_indexes").run(),await e.prepare("DROP VIEW IF EXISTS pg_catalog__pg_index").run()),await de(e,"__edgepg_pg_objects",n.results,Ne);const t=(await e.prepare(`SELECT phase,schema_name,object_name,view_definition
    FROM __edgepg_pg_objects WHERE kind='view' AND view_definition IS NOT NULL
    ...
    AFTER INSERT ON __edgepg_pg_objects WHEN NEW.kind IN ('view','index') AND NEW.relation_oid IS NULL BEGIN
      UPDATE __edgepg_pg_objects SET relation_oid=(SELECT next_oid FROM __edgepg_oid_allocator WHERE id=1)
        WHERE phase=NEW.phase AND kind=NEW.kind AND schema_name=NEW.schema_name AND object_name=NEW.object_name;
      UPDATE __edgepg_oid_allocator SET next_oid=next_oid+1 WHERE id=1;
    END`).run()}async function Qn(e){await e.batch(k(e,"postgres-catalog-table-storage"))}async function Ze(e){await e.batch(k(e,"postgres-catalog-storage"))}async function en(e){await e.batch(k(e,"postgres-catalog"))}function k(e,n){return[e.prepare(`CREATE TABLE IF NOT EXISTS __edgepg_catalog_meta (
      ...
      ON CONFLICT(key) DO UPDATE SET
        ...
        installed_at=excluded.installed_at`).bind(n,X)]}async function I(e,n){for(let t=0;t<n.length;t+=50)await e.batch(n.slice(t,t+50))}function nn(){return`CREATE VIEW information_schema__columns AS
    ${sn()}`}function tn(){return`CREATE VIEW information_schema__views AS
    SELECT 'app' AS table_catalog,object.schema_name AS table_schema,object.object_name AS table_name,
      COALESCE(object.view_definition,object.definition) AS view_definition,
    ...
    FROM __edgepg_pg_objects AS object WHERE object.kind='view'`}function an(){return`CREATE VIEW pg_catalog__pg_sequences AS
    SELECT CASE WHEN instr(sequence.name,'__')>0 THEN substr(sequence.name,1,instr(sequence.name,'__')-1)
        ELSE 'public' END AS schemaname,
      CASE WHEN instr(sequence.name,'__')>0 THEN substr(sequence.name,instr(sequence.name,'__')+2)
        ELSE sequence.name END AS sequencename,
      'edgepg' AS sequenceowner,sequence.data_type AS data_type,
      CAST(sequence.start_value AS TEXT) AS start_value,
      CAST(sequence.min_value AS TEXT) AS min_value,
      CAST(sequence.max_value AS TEXT) AS max_value,
      CAST(sequence.increment_value AS TEXT) AS increment_by,
      sequence.cycle AS cycle,CAST(sequence.cache_value AS TEXT) AS cache_size,
      CASE WHEN sequence.called=1 THEN CAST(sequence.current_value AS TEXT) ELSE NULL END AS last_value
    FROM __edgepg_sequences AS sequence`}function _n(){return`CREATE VIEW pg_catalog__pg_attrdef AS
    SELECT 2604 AS tableoid,
      ...
      ${on("json_extract(tables.current_defaults_json, '$[' || column_name.key || ']')","json_extract(COALESCE(tables.current_pg_types_json,tables.pg_types_json), '$[' || column_name.key || ']')")} AS adbin
    FROM __edgepg_pg_tables AS tables
    JOIN json_each(COALESCE(tables.current_columns_json,tables.columns_json)) AS column_name
    WHERE json_type(tables.current_defaults_json, '$[' || column_name.key || ']') IS NOT NULL
      AND json_type(tables.current_defaults_json, '$[' || column_name.key || ']') <> 'null'`}function on(e,n){const t=`CAST(${e} AS TEXT)`;return`CASE ${n?`WHEN lower(replace(trim(CAST(${n} AS TEXT)), 'pg_catalog.', '')) IN ('boolean','bool')
    ...
    ELSE ${t} END`}function sn(){const e="json_extract(tables.current_defaults_json, '$[' || column_name.key || ']')",n="physical_column.dflt_value",t=`COALESCE(json_extract(tables.current_not_null_json,
      '$[' || column_name.key || ']'), 0)=1
    OR EXISTS (SELECT 1 FROM __edgepg_not_null_constraints AS not_null_constraint
      WHERE not_null_constraint.table_name=tables.physical_name
        AND not_null_constraint.column_name=column_name.value)
    OR EXISTS (SELECT 1 FROM __edgepg_unique_constraints AS primary_constraint,
        json_each(primary_constraint.columns_json) AS primary_column
      WHERE primary_constraint.table_name=tables.physical_name
        AND primary_constraint.kind='primary' AND primary_column.value=column_name.value)`;return`${Le({isNullableSql:`CASE WHEN ${t}
        THEN 'NO' ELSE 'YES' END`,defaultSql:e,fromSql:ue(),whereSql:"WHERE tables.current_not_null_json IS NOT NULL AND tables.current_defaults_json IS NOT NULL"})}
    UNION ALL
    ${Le({isNullableSql:`CASE WHEN tables.current_not_null_json IS NOT NULL
        THEN CASE WHEN ${t} THEN 'NO' ELSE 'YES' END
      WHEN COALESCE(physical_column.[notnull],0)=1 THEN 'NO' ELSE 'YES' END`,defaultSql:`CASE WHEN tables.current_defaults_json IS NOT NULL THEN ${e} ELSE ${n} END`,fromSql:`${ue()}
    ...
    UNION ALL
    ${En()}`}function ue(){const e=`CASE WHEN object.schema_name IN ('','public') THEN object.object_name
    ...
    JOIN json_each(COALESCE(tables.current_columns_json,tables.columns_json)) AS column_name
    JOIN json_each(COALESCE(tables.current_pg_types_json,tables.pg_types_json)) AS pg_type ON pg_type.key=column_name.key`}function Le(e){const n="lower(replace(CAST(pg_type.value AS TEXT), 'pg_catalog.', ''))",t=`(CASE WHEN instr(${n}, '(')>0
    THEN trim(substr(${n}, 1, instr(${n}, '(')-1)) ELSE ${n} END)`,a=`(CASE WHEN ${t}='numeric_text' THEN 'numeric' ELSE ${t} END)`,o=`(CASE WHEN ${a}='varchar'
    THEN 'character varying' ELSE ${a} END)`,p=bn("pg_type.value"),l=Hn("pg_type.value"),s=`(SELECT sequence_default.mode FROM __edgepg_sequence_defaults AS sequence_default
    WHERE sequence_default.table_name=tables.physical_name AND sequence_default.column_name=column_name.value
      AND sequence_default.mode IN ('always','default') LIMIT 1)`,m=`(SELECT sequence_default.sequence_name FROM __edgepg_sequence_defaults AS sequence_default
    WHERE sequence_default.table_name=tables.physical_name AND sequence_default.column_name=column_name.value
      AND sequence_default.mode IN ('always','default') LIMIT 1)`,g=E=>`(SELECT CAST(sequence.${E} AS TEXT) FROM __edgepg_sequences AS sequence
    WHERE sequence.name=${m})`;return`SELECT 'app' AS table_catalog, tables.schema_name AS table_schema, tables.table_name,
      CAST(column_name.value AS TEXT) AS column_name,
      COALESCE(CAST(json_extract(tables.current_attnums_json, '$[' || column_name.key || ']') AS INTEGER),
        CAST(column_name.key AS INTEGER)+1) AS ordinal_position,
      CASE WHEN ${s} IS NOT NULL THEN 'NO' ELSE ${e.isNullableSql} END AS is_nullable,
      ...
      ${a} AS udt_name,
      NULL AS character_maximum_length, ${p} AS numeric_precision,
      ${l} AS numeric_scale,
      ...
      CASE WHEN ${s} IS NULL THEN 'NO' ELSE 'YES' END AS is_identity,
      CASE ${s} WHEN 'always' THEN 'ALWAYS' WHEN 'default' THEN 'BY DEFAULT' ELSE NULL END AS identity_generation,
      ${g("start_value")} AS identity_start,
      ${g("increment_value")} AS identity_increment,
      ${g("max_value")} AS identity_maximum,
      ${g("min_value")} AS identity_minimum,
      CASE WHEN ${s} IS NULL THEN 'NO'
        WHEN (SELECT sequence.cycle FROM __edgepg_sequences AS sequence WHERE sequence.name=${m})=1
    ...
    ${e.fromSql}
    ${e.whereSql}`}function En(){const e="upper(trim(COALESCE(physical_column.type,'')))",n=`(CASE
    ...
    WHERE materialized.relation_name<>''`}function rn(){const e=w("entry.grantor");return`CREATE VIEW pg_catalog__pg_namespace AS
    WITH acl_entries AS (
    ...
    raw_acl AS (
      SELECT entry.schema_name,entry.owner_name,entry.grantor,entry.grantee,${`CASE WHEN entry.grantee='public' THEN '' ELSE ${w("entry.grantee")} END`} || '='
    ...
    FROM __edgepg_schemas AS schema LEFT JOIN acl_text ON acl_text.schema_name=schema.name`}function ln(){return`CREATE VIEW pg_catalog__pg_proc AS
    ...
    FROM (VALUES ${Be.map(([n,t,a,o,p,l])=>`(${n},'${t}',${a},'${JSON.stringify(o)}','${p}',${l?1:0})`).join(",")}) AS builtin_proc
    LEFT JOIN pg_catalog__pg_type AS pg_catalog_type ON pg_catalog_type.oid=column3`}function pn(){return`CREATE VIEW pg_catalog__pg_aggregate AS
    SELECT aggregate.aggregate_oid AS aggfnoid,'n' AS aggkind,0 AS aggnumdirectargs,
      ...
      NULL AS aggminitval
    FROM __edgepg_aggregates AS aggregate`}function cn(e=n=>`COALESCE((SELECT rowid+20000 FROM __edgepg_roles WHERE name=${n}),0)`){const n=w("entry.grantor");return`CREATE VIEW pg_catalog__pg_default_acl AS
    ...
    raw_acl AS (
      SELECT entry.owner_role,entry.schema_name,entry.object_type,entry.grantee,entry.grantor,
        ${`CASE WHEN entry.grantee='public' THEN '' ELSE ${w("entry.grantee")} END`} || '=' || entry.privileges || '/' || ${n} AS item
      FROM acl_entries AS entry),
    ordered_acl AS (
    ...
    LEFT JOIN acl_text ON acl_text.owner_role=groups.owner_role AND acl_text.schema_name=groups.schema_name
      AND acl_text.object_type=groups.object_type`}function mn(){const e=w("entry.grantor");return`CREATE VIEW pg_catalog__pg_database AS
    ...
    raw_acl AS (
      SELECT entry.database_name,entry.owner_name,entry.grantor,entry.grantee,entry.first_ordinal,
        ${`CASE WHEN entry.grantee='public' THEN '' ELSE ${w("entry.grantee")} END`} || '=' || entry.privileges || '/' || ${e} AS item
      FROM acl_entries AS entry),
    ordered_acl AS (
    ...
    WHERE NOT EXISTS (SELECT 1 FROM __edgepg_databases WHERE name='postgres')`}function gn(e=n=>`COALESCE((SELECT rowid+20000 FROM __edgepg_roles WHERE name=${n}),10)`){return`CREATE VIEW pg_catalog__pg_publication AS
    ...
    FROM __edgepg_publications AS publication`}function Nn(){return`CREATE VIEW pg_catalog__pg_publication_tables AS
    ...
    WHERE publication.all_tables<>1`}function Sn(e=n=>`COALESCE((SELECT rowid+20000 FROM __edgepg_roles WHERE name=${n}),10)`){return`CREATE VIEW pg_catalog__pg_subscription AS
    SELECT subscription.rowid+32000 AS oid,
      ...
      NULL AS suborigin,
      NULL AS subfailover
    FROM __edgepg_subscriptions AS subscription`}function Tn(e=n=>`COALESCE((SELECT rowid+20000 FROM __edgepg_roles WHERE name=${n}),10)`){return`CREATE VIEW pg_catalog__pg_tablespace AS
    ...
    FROM __edgepg_catalog_objects WHERE object_type='tablespace'`}function dn(e=n=>`COALESCE((SELECT rowid+20000 FROM __edgepg_roles WHERE name=${n}),10)`){return`CREATE VIEW pg_catalog__pg_foreign_data_wrapper AS
    ...
    FROM __edgepg_catalog_objects WHERE object_type='foreign_data_wrapper'`}function An(e=n=>`COALESCE((SELECT rowid+20000 FROM __edgepg_roles WHERE name=${n}),10)`){const n="'FOREIGN DATA WRAPPER '",t="' OPTIONS '",o=`COALESCE(NULLIF(server.fdw_name,''), ${`trim(CASE
    ...
    WHERE server.object_type='foreign_server'`}function un(e=n=>`CASE WHEN ${n}='public' THEN 0
    ...
    WHERE mapping.object_type='user_mapping'`}function Ln(){const e="json_extract(mapping.name,'$[0]')",n="json_extract(mapping.name,'$[1]')";return`CREATE VIEW pg_catalog__pg_user_mappings AS
    ...
    WHERE mapping.object_type='user_mapping'`}function On(){const e="' SERVER '",n="' OPTIONS '";return`CREATE VIEW pg_catalog__pg_foreign_table AS
    ...
    ELSE '' END)`})
    WHERE tables.persistence='foreign'`}function yn(e=n=>`COALESCE((SELECT rowid+20000 FROM __edgepg_roles WHERE name=${n}),10)`){const n=D("domains.base_type"),t=Je.map(o=>`(${o.join(",")})`).join(","),a=`COALESCE(NULLIF(${W(n,!1)},0),
    ...
    custom_relation_types(typname,typtype,oid,typarray,typnamespace,typowner,typbasetype,typelem,typrelid,typnotnull) AS (
      ...
      SELECT tables.table_name,'c',${F}+tables.relation_oid,
        ${me}+tables.relation_oid,schemas.oid,
      ...
      UNION ALL
      SELECT '_' || tables.table_name,'b',${me}+tables.relation_oid,0,schemas.oid,
    ...
    FROM effective`}function Cn(){const e=D("json_extract(types.values_json,'$.subtype')"),n=W(e),t=a=>`COALESCE((SELECT routine.routine_oid
    ...
    THEN trim(substr(${n}, 1, instr(${n}, '(')-1)
      || substr(${n}, instr(${n}, ')')+1)) ELSE trim(${n}) END)`;return`(CASE WHEN ${t}='numeric_text' THEN 'numeric' ELSE ${t} END)`}function _e(e){const n=`replace(lower(replace(CAST(${e} AS TEXT), 'pg_catalog.', '')), ' ', '')`,t=`instr(${n}, '(')`,a=`instr(${n}, ',')`,o=`instr(${n}, ')')`,s=`${`${`(CASE WHEN ${t}>0 THEN substr(${n},1,${t}-1) ELSE ${n} END)`} IN ('numeric','numeric_text','decimal')`} AND ${t}>0 AND ${a}>${t} AND ${o}>${a}`,m=`CAST(substr(${n},${t}+1,${a}-${t}-1) AS INTEGER)`,g=`CAST(substr(${n},${a}+1,${o}-${a}-1) AS INTEGER)`;return{valid:s,precision:m,scale:g}}function bn(e){const n=_e(e);return`(CASE WHEN ${n.valid} THEN ${n.precision} ELSE NULL END)`}function Hn(e){const n=_e(e);return`(CASE WHEN ${n.valid} THEN ${n.scale} ELSE NULL END)`}function oe(e){const n=_e(e);return`(CASE WHEN ${n.valid}
    THEN 4 + (${n.precision} << 16) + (${n.scale} & 2047) ELSE -1 END)`}function w(e){return`(CASE WHEN ${e}='' OR ${e} GLOB '*[^A-Za-z0-9_]*'
    THEN '"' || replace(replace(${e}, '\\', '\\\\'), '"', '""') || '"'
    ELSE ${e} END)`}function Zn(e,n){return e.prepare(`INSERT INTO __edgepg_pg_tables
    ...
    ON CONFLICT(schema_name, table_name) DO UPDATE SET physical_name=excluded.physical_name,
      ...
      foreign_column_options_json=excluded.foreign_column_options_json`).bind(n.schema,n.name,n.physicalName,n.definition,JSON.stringify(n.columns),JSON.stringify(n.pgTypes),JSON.stringify(n.exportKey),JSON.stringify(n.generatedColumns||[]),n.persistence||"permanent",n.foreignServerName||null,JSON.stringify(n.foreignOptions||[]),JSON.stringify(n.foreignColumnOptions||{}),JSON.stringify(n.notNull||n.columns.map(()=>!1)),JSON.stringify(In(n.pgTypes,n.defaults||n.columns.map(()=>null))),JSON.stringify(n.reloptions||[]),JSON.stringify(n.columns.map((t,a)=>a+1)))}function et(e,n){let t="COALESCE(current_columns_json,columns_json)",a="COALESCE(current_pg_types_json,pg_types_json)",o=`COALESCE(current_attnums_json,
    (SELECT json_group_array(CAST(key AS INTEGER)+1) FROM json_each(COALESCE(current_columns_json,columns_json))))`,p=`CASE WHEN max_attnum>0 THEN max_attnum
    ELSE COALESCE((SELECT MAX(CAST(value AS INTEGER)) FROM json_each(${o})),0) END`,l=`COALESCE(current_not_null_json,
    (SELECT json_group_array(0) FROM json_each(COALESCE(current_columns_json,columns_json))))`,s=`COALESCE(current_defaults_json,
    (SELECT json_group_array(NULL) FROM json_each(COALESCE(current_columns_json,columns_json))))`,m="export_key_json",g="COALESCE(generated_columns_json,'[]')";for(const E of n.shapeChanges){const i=O(E.column);if(E.action==="add"){const r=`(${p}+1)`;t=`json_insert(${t}, '$[#]', ${i})`,a=`json_insert(${a}, '$[#]', ${O(E.pgType)})`,o=`json_insert(${o}, '$[#]', ${r})`,p=r,l=`json_insert(${l}, '$[#]', ${E.notNull?1:0})`,s=`json_insert(${s}, '$[#]', ${E.defaultSql===void 0?"NULL":O(E.defaultSql)})`}else{const r=`(SELECT key FROM json_each(${t}) WHERE value=${i} LIMIT 1)`;E.action==="drop"?(t=`CASE WHEN ${r} IS NULL THEN ${t}
          ...
          ELSE json_remove(${l}, '$[' || ${r} || ']') END`,s=`CASE WHEN ${r} IS NULL THEN ${s}
          ELSE json_remove(${s}, '$[' || ${r} || ']') END`,m=`CASE WHEN EXISTS (SELECT 1 FROM json_each(${m}) WHERE value=${i})
          THEN json_remove(${m}, '$[' || (SELECT key FROM json_each(${m}) WHERE value=${i} LIMIT 1) || ']')
          ELSE ${m} END`):E.action==="rename"?(t=`json_set(${t}, '$[' || ${r} || ']', ${O(E.newColumn)})`,m=`CASE WHEN EXISTS (SELECT 1 FROM json_each(${m}) WHERE value=${i})
          THEN json_set(${m}, '$[' || (SELECT key FROM json_each(${m}) WHERE value=${i} LIMIT 1) || ']', ${O(E.newColumn)})
          ELSE ${m} END`,g=`(SELECT COALESCE(json_group_array(json_object(
          'column', CASE WHEN json_extract(value,'$.column')=${i} THEN ${O(E.newColumn)}
            ELSE json_extract(value,'$.column') END,
          'expressionSql', replace(json_extract(value,'$.expressionSql'),
            ${O(`"${E.column.replaceAll('"','""')}"`)},
            ${O(`"${E.newColumn.replaceAll('"','""')}"`)}),
          'stored', json_extract(value,'$.stored'))), '[]') FROM json_each(${g}))`):E.action==="type"?a=`json_set(${a}, '$[' || ${r} || ']', ${O(E.pgType)})`:E.action==="set-not-null"?l=`json_set(${l}, '$[' || ${r} || ']', 1)`:E.action==="drop-not-null"?l=`json_set(${l}, '$[' || ${r} || ']', 0)`:E.action==="set-default"?s=`json_set(${s}, '$[' || ${r} || ']',
        ${E.defaultSql===void 0?"NULL":O(E.defaultSql)})`:E.action==="drop-default"&&(s=`json_set(${s}, '$[' || ${r} || ']', NULL)`)}}return e.prepare(`UPDATE __edgepg_pg_tables
    SET alterations_json=json_insert(COALESCE(alterations_json, '[]'), '$[#]', ?3),
        ...
        current_attnums_json=${o},max_attnum=${p},
        current_not_null_json=${l}, current_defaults_json=${s}, export_key_json=${m},
        generated_columns_json=${g}
    WHERE schema_name=?1 AND table_name=?2`).bind(n.schema,n.name,n.definition,JSON.stringify(n.shapeChanges))}function nt(e,n,t="edgepg"){return e.prepare(`INSERT INTO __edgepg_pg_objects(
      ...
      dependencies_json,routine_dependencies_json,reloptions_json,index_nulls_not_distinct,ordinal)
    VALUES (?1,?2,?3,?4,?5,?6,?7,?8,
      COALESCE(?13,(SELECT count(*) FROM pragma_table_info(?9))),?14,?15,?10,?11,?12,
      ?16,COALESCE((SELECT MAX(ordinal)+1 FROM __edgepg_pg_objects WHERE phase=?1), 1))
    ON CONFLICT(phase, kind, schema_name, object_name) DO UPDATE SET
      ...
      reloptions_json=excluded.reloptions_json,index_nulls_not_distinct=excluded.index_nulls_not_distinct`).bind(n.phase,n.kind,n.schema,n.name,t,n.definition,n.viewDefinition||null,n.compactViewDefinition||n.viewDefinition||null,n.schema==="public"?n.name:`${n.schema}__${n.name}`,JSON.stringify(n.dependencies||[]),JSON.stringify(n.routineDependencies||[]),JSON.stringify(n.reloptions||[]),n.relationNatts??null,JSON.stringify(n.relationPgTypes||[]),JSON.stringify(n.relationCollations||[]),n.indexNullsNotDistinct?1:0)}function Rn(){const e=a=>`'"' || replace(${a}, '"', '""') || '"'`,n=`${e("tables.schema_name")} || '.' || ${e("tables.table_name")}`,t=`(SELECT group_concat(${e("CAST(constraint_column.value AS TEXT)")}, ', ')
    ...
    JOIN json_each(object.dependencies_json) AS dependency
    JOIN __edgepg_pg_tables AS tables ON tables.physical_name=CAST(dependency.value AS TEXT)
    WHERE object.kind='index' AND object.relation_oid IS NOT NULL`}function tt(e,n){const t=JSON.parse(e);if(!Array.isArray(t)||!t.every(a=>typeof a=="string"))throw new TypeError(`${n} is not a string array`);return t}async function jn(e){const n=await e.prepare(`SELECT schema_name,table_name,physical_name,
    ...
    FROM __edgepg_pg_tables ORDER BY ordinal`).all(),t=[];for(const a of n.results){const o=`${a.schema_name}.${a.table_name}`,p=V(a.columns_json),l=V(a.pg_types_json);let s;if(p&&l&&p.length===l.length)try{s=hn(p,l,a.shape_changes_json,o)}catch{s=void 0}const g=(await e.prepare(`PRAGMA table_info(${be(a.physical_name)})`).all()).results.filter(_=>!_.name.startsWith("__edgepg_")),E=g.map(_=>_.name),i=V(a.current_columns_json),r=V(a.current_pg_types_json),L=fn(a.current_attnums_json),C=!!(i&&r&&i.length===r.length&&(!E.length||ie(i,E)));let N,T,S,c;if(C)N=i,T=r,L&&L.length===N.length&&L.every(_=>Number.isSafeInteger(_)&&_>0)?(S=L,c=Math.max(Number(a.max_attnum)||0,...S,0)):s&&ie(N,s.columns)?(S=s.attnums,c=s.maxAttnum):(S=N.map((_,z)=>z+1),c=S.length);else if(s&&(!E.length||ie(s.columns,E)))({columns:N,pgTypes:T,attnums:S,maxAttnum:c}=s);else{N=E.length?E:p||[];const _=se(i,r),z=s?se(s.columns,s.pgTypes):new Map,ye=se(p,l);T=g.length?g.map(M=>_.get(M.name)||z.get(M.name)||ye.get(M.name)||String(M.type||"text").toLowerCase()):l||[],S=N.map((M,Ce)=>Ce+1),c=S.length}if(!N.length||N.length!==T.length)continue;const R=P(a.current_not_null_json),j=P(a.current_defaults_json),U=!C||!L||L.length!==N.length||L.some(_=>!Number.isSafeInteger(_)||_<=0)||Number(a.max_attnum)<Math.max(...S,0),Y=U||!R||R.length!==N.length||!j||j.length!==N.length;if(!U&&!Y)continue;const x=new Map(g.map(_=>[_.name,_])),v=R?.length===N.length?R:N.map(_=>Number(x.get(_)?.notnull||0)===1),Ee=j?.length===N.length?j:N.map(_=>Oe(x.get(_)?.dflt_value??null));t.push(e.prepare(`UPDATE __edgepg_pg_tables SET
      ...
      WHERE schema_name=?1 AND table_name=?2`).bind(a.schema_name,a.table_name,JSON.stringify(N),JSON.stringify(T),JSON.stringify(S),c,JSON.stringify(v),JSON.stringify(Ee)))}t.length&&await e.batch(t)}function P(e){if(typeof e=="string")try{const n=JSON.parse(e);return Array.isArray(n)?n:void 0}catch{return}}function V(e){const n=P(e);return n?.every(t=>typeof t=="string")?n:void 0}function fn(e){const n=P(e);return n?.every(t=>typeof t=="number")?n:void 0}function ie(e,n){return e.length===n.length&&e.every((t,a)=>t===n[a])}function se(e,n){const t=new Map;if(!e||!n||e.length!==n.length)return t;for(const[a,o]of e.entries())t.set(o,n[a]);return t}function hn(e,n,t,a){if(e.length!==n.length)throw new TypeError(`${a} initial column/type width mismatch`);const o=JSON.parse(t);if(!Array.isArray(o))throw new TypeError(`${a} shape changes are not an array`);const p=o.flatMap(i=>Array.isArray(i)?i:[i]),l=[...e],s=[...n],m=e.map((i,r)=>r+1);let g=m.length+1;const E=new Map;for(const i of p){if(!i||typeof i!="object"||typeof i.action!="string"||typeof i.column!="string")throw new TypeError(`${a} contains an invalid shape change`);const r=l.indexOf(i.column);if(i.action==="add"){if(typeof i.pgType!="string"||l.includes(i.column))throw new TypeError(`${a} contains an invalid added column`);l.push(i.column),s.push(i.pgType),m.push(g++)}else if(i.action==="drop"){if(r<0)throw new TypeError(`${a} drops an unknown column ${i.column}`);l.splice(r,1),s.splice(r,1),m.splice(r,1)}else if(i.action==="rename"){if(r<0||typeof i.newColumn!="string"||l.includes(i.newColumn))throw new TypeError(`${a} contains an invalid column rename`);l[r]=i.newColumn;for(const[L,C]of E)C===i.column&&E.set(L,i.newColumn);E.set(i.column,i.newColumn)}else if(i.action==="type"){if(r<0||typeof i.pgType!="string")throw new TypeError(`${a} changes an unknown column type`);s[r]=i.pgType}else{if(["set-not-null","drop-not-null","set-default","drop-default"].includes(String(i.action)))continue;throw new TypeError(`${a} contains an unsupported shape change`)}}return{columns:l,pgTypes:s,attnums:m,maxAttnum:g-1,renames:E}}function O(e){return`'${e.replaceAll("'","''")}'`}function Oe(e){return e===null?null:/\bstrftime\s*\([^)]*['"]now['"]/iu.test(e)||e.toUpperCase()==="CURRENT_TIMESTAMP"?"now()":/^\(?\s*\(?random\(\)\s*&\s*9223372036854775807\)?\s*\/\s*9223372036854775808\.0\s*\)?\s*\)?$/iu.test(e.trim())?"random()":e}function In(e,n){return n.map((t,a)=>Wn(t,e[a]||""))}function Wn(e,n){if(e===null)return null;e=Oe(e);const t=n.toLowerCase().replace(/^pg_catalog\./u,"");if(t==="boolean"||t==="bool"){const a=e.trim().toLowerCase();if(a==="1"||a==="true")return"true";if(a==="0"||a==="false")return"false"}return/^bit(?:\s+varying)?(?:\s*\(\s*\d+\s*\))?$/iu.test(t)&&!/::(?:"bit"|bit\s+varying)$/iu.test(e.trim())?`${e}::${/^bit\s+varying\b/iu.test(t)?"bit varying":'"bit"'}`:e}export{F as RELATION_TYPE_OID_OFFSET,Kn as ensurePostgresObjectCatalogStorage,Ve as ensurePostgresTableCatalogStorage,sn as informationSchemaColumnsBodySql,nn as informationSchemaColumnsViewSql,tn as informationSchemaViewsViewSql,Ye as installPostgresCatalog,In as normalizeCatalogDefaults,pn as pgAggregateViewSql,_n as pgAttributeDefaultViewSql,bn as pgAttributeNumericPrecisionSql,Hn as pgAttributeNumericScaleSql,ae as pgAttributePhysicalMetadataSql,oe as pgAttributeTypeModSql,D as pgAttributeTypeNameSql,mn as pgDatabaseViewSql,cn as pgDefaultAclViewSql,dn as pgForeignDataWrapperViewSql,An as pgForeignServerViewSql,On as pgForeignTableViewSql,Rn as pgIndexesViewSql,rn as pgNamespaceViewSql,ln as pgProcViewSql,Nn as pgPublicationTablesViewSql,gn as pgPublicationViewSql,Cn as pgRangeViewSql,an as pgSequencesViewSql,Sn as pgSubscriptionViewSql,Tn as pgTablespaceViewSql,W as pgTypeOidSql,yn as pgTypeViewSql,un as pgUserMappingViewSql,Ln as pgUserMappingsViewSql,et as postgresCatalogAlterationStatement,on as postgresCatalogDefaultExpressionSql,nt as postgresCatalogObjectStatement,Zn as postgresCatalogTableStatement,zn as postgresRelationCatalogStorageCached};
